...
*/
FSkeletalMeshMergeModified::FSkeletalMeshMergeModified(USkeletalMesh* InMergeMesh,
	const TArray<USkeletalMesh*>& InSrcMeshList,
	const TArray<FSkelMeshMergeSectionMapping>& InForceSectionMapping,
	const TArray<FSkelMeshMaterialOverride>& InMaterialOverrides,
	...
	, MaterialOverrides(InMaterialOverrides)
...
*/
bool FSkeletalMeshMergeModified::DoMerge(TArray<FRefPoseOverrideMod>* RefPoseOverrides /* = nullptr */)
{
...
void FSkeletalMeshMergeModified::MergeSkeleton(const TArray<FRefPoseOverrideMod>* RefPoseOverrides /* = nullptr */)
{
...
bool FSkeletalMeshMergeModified::FinalizeMesh()
{
	bool Result = true;
	...
	if (MaxNumLODs == -1)
	{
		UE_LOG(LogSkeletalMesh, Warning, TEXT("FSkeletalMeshMergeModified: Invalid source mesh list"));
		return false;
	}
...
*/
void FSkeletalMeshMergeModified::MergeBoneMap(TArray<FBoneIndexType>& MergedBoneMap, TArray<FBoneIndexType>& BoneMapToMergedBoneMap, const TArray<FBoneIndexType>& BoneMap)
{
...
*/
void FSkeletalMeshMergeModified::GenerateNewSectionArray(TArray<FNewSectionInfo>& NewSectionArray, int32 LODIdx)
{
	...
	for (int32 MeshIdx = 0; MeshIdx < SrcMeshList.Num(); MeshIdx++)
	{
		...
		if (SrcMesh)
		{
			...
			for (int32 SectionIdx = 0; SectionIdx < SrcLODData.RenderSections.Num(); SectionIdx++)
			{
				...
				if (MaterialOverrides.Num() == SrcMeshList.Num() &&
					MaterialOverrides.IsValidIndex(MeshIdx) &&
					MaterialOverrides[MeshIdx].Materials.IsValidIndex(SectionIdx))
				{
					UE_LOG(LogTemp, Warning, TEXT("Overriding Materials In Merge"));
					// create a new section entry
					const FSkeletalMaterial& SkeletalMaterial = MaterialOverrides[MeshIdx].Materials[SectionIdx];
					...
					FNewSectionInfo& NewSectionInfo = *new(NewSectionArray) FNewSectionInfo(SkeletalMaterial.MaterialInterface, MaterialId, MaterialSlotName, UVChannelData);
...
void FSkeletalMeshMergeModified::CopyVertexFromSource(VertexDataType& DestVert, const FSkeletalMeshLODRenderData& SrcLODData, int32 SourceVertIdx, const FMergeSectionInfo& MergeSectionInfo)
{
...
*/
template<typename VertexDataType>
void FSkeletalMeshMergeModified::GenerateLODModel(int32 LODIdx)
{
...
* @return true if succeeded
*/
bool FSkeletalMeshMergeModified::ProcessMergeMesh()
{
	bool Result = true;
...
int32 FSkeletalMeshMergeModified::CalculateLodCount(const TArray<USkeletalMesh*>& SourceMeshList) const
{
...
void FSkeletalMeshMergeModified::BuildReferenceSkeleton(const TArray<USkeletalMesh*>& SourceMeshList, FReferenceSkeleton& RefSkeleton, const USkeleton* SkeletonAsset)
{
	RefSkeleton.Empty();
...
void FSkeletalMeshMergeModified::OverrideReferenceSkeletonPose(const TArray<FRefPoseOverrideMod>& PoseOverrides, FReferenceSkeleton& TargetSkeleton, const USkeleton* SkeletonAsset)
{
	for (int32 i = 0, PoseMax = PoseOverrides.Num(); i < PoseMax; ++i)
	{
		const FRefPoseOverrideMod& PoseOverride = PoseOverrides[i];
		...
		for (int32 j = 0, BoneMax = PoseOverride.Overrides.Num(); j < BoneMax; ++j)
		{
			...
			{
				bool bOverrideBone = (PoseOverride.Overrides[j].OverrideMode == FRefPoseOverrideMod::ChildrenOnly) ? false : true;
				...
				bool bOverrideChildren = (PoseOverride.Overrides[j].OverrideMode == FRefPoseOverrideMod::BoneOnly) ? false : true;
...
bool FSkeletalMeshMergeModified::OverrideReferenceBonePose(int32 SourceBoneIndex, const FReferenceSkeleton& SourceSkeleton, FReferenceSkeletonModifier& TargetSkeleton)
{
...
void FSkeletalMeshMergeModified::ReleaseResources(int32 Slack)
{
...
bool FSkeletalMeshMergeModified::AddSocket(const USkeletalMeshSocket* NewSocket, bool bIsSkeletonSocket)
{
...
void FSkeletalMeshMergeModified::AddSockets(const TArray<USkeletalMeshSocket*>& NewSockets, bool bAreSkeletonSockets)
{
...
void FSkeletalMeshMergeModified::BuildSockets(const TArray<USkeletalMesh*>& SourceMeshList)
{
...
void FSkeletalMeshMergeModified::OverrideSocket(const USkeletalMeshSocket* SourceSocket)
{
...
void FSkeletalMeshMergeModified::OverrideBoneSockets(const FName& BoneName, const TArray<USkeletalMeshSocket*>& SourceSocketList)
{
...
void FSkeletalMeshMergeModified::OverrideMergedSockets(const TArray<FRefPoseOverrideMod>& PoseOverrides)
{
	for (int32 i = 0, PoseMax = PoseOverrides.Num(); i < PoseMax; ++i)
	{
		const FRefPoseOverrideMod& PoseOverride = PoseOverrides[i];
